...
func TestInitializeRejectsUnexpectedComponentPath(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "unexpected agent-plugins component path") {
		t.Fatalf("Initialize() err=%v", err)
	}
	if len(runner.mutations) != 0 {
...
func makeInstallation(t *testing.T) string {
	...
	receipt := installer.Receipt{
		SchemaVersion: "agx.receipt/v1", InstallationID: "install-test", BundleID: "bundle-test", Phase: "configured",
		Components: []installer.Component{
			{Name: "agent-control", Path: "components/agent-control"},
			{Name: "agent-plugins", Path: "components/agent-plugins"},
		},
